...
void rwlock_acquire_readlock(rwlock_t *rw) {
  /*
  1. delay capture of rw->lock, so that the scheduler can be tweaked to prefer switching to the writer.
  2. only at the beginning, so the reader concurrency is not destroyed. see ostep-hw README.md
  */
...
void rwlock_acquire_writelock(rwlock_t *rw) {
  /*
  Make reader and writer can race more equally but not totally equal.
  totally equal see csapp 12.19/20.c
  */
