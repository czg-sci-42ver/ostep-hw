...
struct user_data server_data_arr[LISTEN_BACKLOG];
...
UDP_Open(char *hostName, char *port, int server)
{
    ...
    */
    ...
    if (!server && io_uring_queue_init(LISTEN_BACKLOG, &server_ring, IORING_SETUP_SQPOLL))
        handle_error("io_uring_queue_init"); 
    #endif
...
void check_cqe(struct io_uring_cqe *cqe,struct io_uring *ring){
    if (io_uring_wait_cqe(ring, &cqe))
        handle_error("io_uring_wait_cqe");
    if (cqe->res < 0) {
        fprintf(stderr, "I/O error: %s\n", strerror(-cqe->res));
...
void prep_recv(struct io_uring *ring, int cfd, int index) {
  struct io_uring_sqe *sqe = io_uring_get_sqe(ring);
  if (sqe == NULL)
    handle_error("io_uring_get_sqe");

  server_data_arr[index].io_op = IORING_OP_RECV;
  server_data_arr[index].socket_fd = cfd;
  memset(server_data_arr[index].buf, 0, BUFSIZ);
  io_uring_prep_recv(sqe, cfd, server_data_arr[index].buf, BUFSIZ, 0);
  io_uring_sqe_set_data(sqe, &server_data_arr[index]);
  if (io_uring_submit(ring) < 0)
    handle_error("io_uring_submit");
}
...
UDP_Read(int sfd, char *buffer, struct sockaddr *peer_addr, socklen_t *peer_addr_len)
{
    ...
    prep_recv(&server_ring, sfd, server_seq_cnt);
    struct io_uring_cqe *cqe;
    check_cqe(cqe,&server_ring);
    struct user_data *data;
    data = (struct user_data *)io_uring_cqe_get_data(cqe);
    switch (data->io_op) {
        case IORING_OP_RECV:
