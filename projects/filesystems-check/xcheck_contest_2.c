        ...
        if (strcmp(de.name, ".") == 0) {
          ...
          */
          ...
          if (i == ROOTINO && de.inum == ROOTINO)
            ...
            int find=0;
            /*
            since 'find dir inode' only has root 1, so here no test run originally.
            with `~/ostep-hw/projects/filesystems-check/xv6_patch/mkfs.c.lost_found.patch`, `lost_found` is checked.
            */
