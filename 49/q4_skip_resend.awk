...
function update_contents(fh,uid,whether_write){
    ...
    if (file_user_off[whether_write][fh][uid]==file_user_old_off[whether_write][fh][uid]) {
        # printf("(%d %d) resend\n",fh,uid)
    }else if (file_user_next_off[whether_write][fh][uid]!=file_user_off[whether_write][fh][uid]) {
