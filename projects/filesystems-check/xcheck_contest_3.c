...
void rsect(void *imgp,void *buf,uint sec){
  memmove(buf, imgp + sec * BSIZE,
                  sizeof(buf));
}

void wsect(void *imgp,void *buf,uint sec){
...
iappend(struct dinode* inodes_ptr, void *imgp, uint save_dir_inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    if(fbn < NDIRECT){
      ...
      rsect(imgp,indirect,din.addrs[NDIRECT]);           
      if(indirect[fbn - NDIRECT] == 0){
        indirect[fbn - NDIRECT] = xint(data_end_block++);
        wsect(imgp,indirect,din.addrs[NDIRECT]);
      }
...
int main(int argc, char *argv[]) {
      ...
      for (int off = 0; off < inode.size; off += sizeof(de)) {
        ...
        if (strcmp(de.name, ".") == 0) {
          ...
          */
          ...
          if (strncmp(de.name, target_dir_str, strlen(target_dir_str)+1)==0) {
            lost_found_dir_inum=i;
          }
          /*
  ...
  */
  ...
  if (repair) {
    #ifdef NOT_MMAP_COPY
    /*same as mkfs.c*/
    int fd=open("fs.img.repair", O_RDWR|O_CREAT|O_TRUNC, 0666);
    assert(fd!=-1);
    /*
    not use sizeof which may stop at ending 0.
    */
    write(fd, imgp, IMG_SIZE);
    assert(close(fd)!=-1);
    #else
    char *dst;
    if ((dst = mmap(NULL, IMG_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0))==MAP_FAILED) {
